...
### Added
- **Web Dashboard** — `rlm dashboard` command starts a web UI for cost tracking
- **MongoDB Cost Logging** — All CLI and MCP server usage automatically logged to MongoDB
- **REST API** — `/api/summary`, `/api/projects`, `/api/logs` endpoints for programmatic access
- **Cost per project** — Track and visualize API costs per project, per day, and per month
- **Clean logs** — Delete usage logs by project, date range, or all at once
- **`rlm-dashboard` binary** — Standalone dashboard server entry point

### Dependencies
- Added `mongodb` for usage tracking database
- Added `express` for web dashboard server
